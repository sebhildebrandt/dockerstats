...
const _error_no_container_id = 'ERROR: no container ID';
const _error_no_containers = 'ERROR: no containers';
  ...
  return new Promise((resolve, reject) => {
    process.nextTick(() => {
      if (containerID) {
        ...
        if (callback) { callback({error: _error_no_container_id}) }
        reject(error);
      }
    });
  });
    ...
    process.nextTick(() => {
      dockerContainers(true).then(result => {
        if (result && Object.prototype.toString.call(result) === '[object Array]' && result.length > 0) {
          ...
          if (callback) { callback({error: _error_no_containers}) }
          reject(error);
        }
      })
    });
