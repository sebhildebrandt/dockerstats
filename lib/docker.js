...
function dockerImagesInspect(imageID, payload) {
	return new Promise((resolve) => {
		process.nextTick(() => {
			...
			const imageIDSanitized = util.sanitizeImageID(imageID);
			if (imageIDSanitized) {
				if (!_docker_socket) {
					_docker_socket = new DockerSocket();
				}
				...
				try {
					...
					if (
						...
						for (const key in _docker_cpu_last_read) {
							if (Object.hasOwn(_docker_cpu_last_read, key)) {
						...
						docker_containers.forEach((element) => {
							...
							if (element.Id && typeof element.Id === "string") {
								workload.push(
									dockerContainerInspect(element.Id.trim(), element),
								);
							}
						});
						if (workload.length) {
...
function dockerContainerInspect(containerID, payload) {
	return new Promise((resolve) => {
		process.nextTick(() => {
			...
			const containerIdSanitized = util.sanitizeContainerID(containerID);
						...
						resolve({
							...
							finishedAt:
								...
								!data.State.FinishedAt.startsWith("0001-01-01")
									...
									: "",
							status:
								data.State && data.State.Health && data.State.Health.Status
							...
							driver: data.Driver || "",
							labels:
								data.Config && data.Config.Labels ? data.Config.Labels : {},
							ports: payload.Ports,
...
function docker_calcCPUPercent(cpu_stats, precpu_stats, id) {
	...
	const precpuTotal =
		precpu_stats.cpu_usage && precpu_stats.cpu_usage.total_usage
	...
	if (!_windows) {
		...
		const systemDelta =
			cpu_stats.system_cpu_usage - (precpu_stats.system_cpu_usage || 0);

		if (systemDelta > 0.0 && cpuDelta > 0.0) {
			if (precpu_stats.online_cpus) {
				cpuPercent =
					(cpuDelta / systemDelta) * precpu_stats.online_cpus * 100.0;
			} else if (
		...
		process.nextTick(() => {
			...
			if (util.isFunction(containerIDs) && !callback) {
				...
				if (containerIDsSanitized !== "*") {
					containerIDsSanitized = util.sanitizeContainerID(containerIDs);
				}
									...
									result.memPercent =
										...
										stats.memory_stats.limit
											? (stats.memory_stats.usage / stats.memory_stats.limit) *
												100.0
											: 0;
									result.cpuPercent = docker_calcCPUPercent(
										stats.cpu_stats,
										stats.precpu_stats,
										containerID,
									);
				...
				_docker_socket.getProcesses(containerIdSanitized, (data) => {
					...
					try {
						if (data && data.Titles && data.Processes) {
							const titles = data.Titles.map((value) => value.toUpperCase());
...
function dockerAll(callback) {
	return new Promise((resolve) => {
		process.nextTick(() => {
			dockerContainers(true).then((data) => {
				const result = (data || []).filter((element) => element && element.id);
				if (result.length > 0) {
					let l = result.length;
					result.forEach((element) => {
						dockerContainerStats(element.id).then((res) => {
							const stats = (res && res[0]) || {};
							element.memUsage = stats.memUsage;
							element.memLimit = stats.memLimit;
							element.memPercent = stats.memPercent;
							element.cpuPercent = stats.cpuPercent;
							element.pids = stats.pids;
							element.netIO = stats.netIO;
							element.blockIO = stats.blockIO;
							element.cpuStats = stats.cpuStats;
							element.precpuStats = stats.precpuStats;
							element.memoryStats = stats.memoryStats;
							element.networks = stats.networks;
