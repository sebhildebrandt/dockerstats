...
const socketTimeout = +process.env.DOCKER_SOCKET_TIMEOUT || 10000;
...
class DockerSocket {
	...
	getProcesses(id, callback) {
		if (id) {
			fetchJson(
				`http:/containers/${id}/top?ps_args=-opid,ppid,pgid,vsz,time,etime,nice,ruser,user,rgroup,group,stat,rss,args`,
				callback,
			);
		} else {
			callback({});
